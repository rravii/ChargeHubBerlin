...
class InMemoryRatingRepository(RatingRepositoryInterface):
    """In-memory rating store used for tests and demos."""
    ...
    def upsert(self, rating: Rating) -> Rating:
        """Insert or replace a rating and maintain lookup indices."""
    ...
    def get_by_station(self, station_id: StationId) -> list[Rating]:
        """Return all ratings for a station, newest last."""
    ...
    def get_by_user_and_station(
        ...
        """Lookup a single rating by user and station."""
    ...
    def get_average_for_station(self, station_id: StationId) -> float | None:
        """Compute the average star rating for a station, or None if empty."""
