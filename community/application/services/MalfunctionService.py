...
from community.domain.events.MalfunctionReportOpenedEvent import (
    MalfunctionReportOpenedEvent,
)
from community.domain.events.DetailsEnteredEvent import DetailsEnteredEvent
from community.domain.events.ReportValidatedAndSubmittedEvent import (
    ReportValidatedAndSubmittedEvent,
)
from community.domain.events.ReportNotifiedToAdminAndOperatorEvent import (
    ReportNotifiedToAdminAndOperatorEvent,
)
from community.domain.events.ReportPublishedToUsersEvent import (
    ReportPublishedToUsersEvent,
)
from community.domain.events.WarningDisplayedToAllUsersEvent import (
    WarningDisplayedToAllUsersEvent,
)
from community.domain.events.PointsAwardedToReporterEvent import (
    PointsAwardedToReporterEvent,
)
from community.domain.events.SolutionProvidedEvent import SolutionProvidedEvent
from community.domain.events.IssuesStatusUpdateReportEvent import (
    IssuesStatusUpdateReportEvent,
)
from community.infrastructure.repositories.IncidentRepositoryInterface import (
    IncidentRepositoryInterface,
)
from community.infrastructure.EventBus import InMemoryEventBus


class MalfunctionService:
    """
    Application service for the 'Report Malfunction' use case.

    Coordinates validation, persistence, and domain events for incident reports.
    """
    ...
    def open_report(
        ...
        """
        Create and persist a new malfunction incident and emit the core events.

        reporter_is_resident controls whether a 'points awarded' event is raised.
        """
        now = datetime.utcnow()
